...
SELECT  t1.order_id,                                                                                                                     
  ...
  select t1.order_id,                                                                                                                    
      ...
      join external_source.customers t3 on t3.customer_id = t1.customer_id;                                                              
...
dwh_update_delta AS ( -- делаем выборку по заказчикам ручной работы, по которым были изменения в DWH. По этим заказчикам данные в витрине нужно будет обновить
    SELECT     
                        ...
                        *,
                        ...
                        FROM ( 
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у мастера. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним JOIN
                                ...
                                INNER JOIN (
                                    SELECT     -- Эта выборка поможет определить самый популярный товар у заказчиков. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет самую популярную категорию товаров у мастера
                                 ...
                                 inner join (select dd.customer_id as customer_id_for_craftsman_id, craftsman_id as top_craftsman_id, count(craftsman_id) as count_craftsman_orders FROM dwh_delta AS dd group by dd.customer_id, craftsman_id) t5 on t5.customer_id_for_craftsman_id = t2.customer_id
                ) AS T4 WHERE T4.rank_count_product = 1 and t4.rank_count_craftsman_orders=1 ORDER BY report_period -- условие помогает оставить в выборке первую по популярности категорию товаров
),
                                    ...
                                    SELECT     -- Эта выборка поможет определить самый популярный товар у мастера. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет самую популярную категорию товаров у мастера
                                            ...
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.product_type
                                                    ORDER BY count_product DESC) AS T3 ON T2.customer_id = T3.customer_id_for_product_type
                                                                                     inner join (select dd.customer_id as customer_id_for_craftsman_id, craftsman_id as top_craftsman_id, count(craftsman_id) as count_craftsman_orders FROM dwh_delta AS dd group by dd.customer_id, craftsman_id) t5 on t5.customer_id_for_craftsman_id = t2.customer_id
                ) AS T4 WHERE T4.rank_count_product = 1 and t4.rank_count_craftsman_orders=1 ORDER BY report_period
)
,
insert_delta as (
...
insert
	into
		...
		select
			greatest(coalesce(MAX(customer_load_dttm),
			NOW()),
			coalesce(MAX(customers_load_dttm),
			NOW()),
			coalesce(MAX(products_load_dttm),
			NOW()))
		from
			dwh_delta
)
select
	'increment datamart'; -- инициализируем запрос CTE
